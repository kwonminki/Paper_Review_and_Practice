'''
python train.py --mode "train" --network "unet" --task "inpainting" --opts "random" 0.5 --learning_type "residual" --train_continue "on" --epochs 1000
'''
#%%
import argparse
...
PATH = HOME_PATH/"UNet_Module_regression"
...
parser.add_argument("--network", default="unet", choices=["unet"], type=str, dest="network")
...
##네트워크 생성
if network == "unet":
    net = UNet(nch=nch, nker=nker, norm=norm, learning_type=learning_type).to(device)
