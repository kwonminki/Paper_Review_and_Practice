...
def save(ckpt_dir, net, optim, epoch, isgan=None):
    ...
    if isgan is not None: #net도 dic 안에 list로 줘야함.
        ...
        for i in range(len(net["generator"])):
            save_dic.update({'netG{}'.format(i) : net["generator"][i].state_dict()})
        for j in range(len(net["discriminator"])):
            save_dic.update({'netD{}'.format(j) : net["discriminator"][j].state_dict()})
        save_dic.update({'optimG' : optim["generator"].state_dict()})
        save_dic.update({'optimD' : optim["discriminator"].state_dict()})
...
def load(ckpt_dir, net, optim, index = -1, isgan=None):
    ...
    dict_model = torch.load('%s/%s' % (ckpt_dir, ckpt_lst[index]), map_location='cuda')

    if isgan is not None:
        for i in range(len(net["generator"])):
            net["generator"][i].load_state_dict(dict_model['netG{}'.format(i)])
            
        for i in range(len(net["discriminator"])):
            net["discriminator"][i].load_state_dict(dict_model['netD{}'.format(i)])

        optim["generator"].load_state_dict(dict_model['optimG'])
        optim["discriminator"].load_state_dict(dict_model['optimD'])
