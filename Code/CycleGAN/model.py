...
class CycleGAN_G(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, norm="inorm"):
        ...
        self.c7s1_64 = ConvBNormRelu2d(in_channels, nker, kernel_size=7, stride=1, padding=(7-1)//2, padding_mode='reflect', norm=norm, relu="relu")
        self.d128 = ConvBNormRelu2d(nker, 2*nker, kernel_size=3, stride=2, padding=(3-1)//2, padding_mode='reflect', norm=norm, relu="relu")
        self.d256 = ConvBNormRelu2d(2*nker, 4*nker, kernel_size=3, stride=2, padding=1, padding_mode='reflect', norm=norm, relu="relu")
        res = []
        for i in range(9):
            res += [ResBlock(4*nker, 4*nker, kernel_size=3, norm=norm, relu="relu", stride=1, padding = (3-1)//2)]
        self.res = nn.Sequential(*res)
        self.u128 = DEConvBNormRelu2d(4*nker, 2*nker, kernel_size=3, stride=2, padding=(3-1)//2, norm=norm, relu="relu") #논문에 stride = 1/2인데 Transposed이므로 역수로 해주면 된다.
        self.u64 = DEConvBNormRelu2d(2*nker, nker, kernel_size=3, stride=2, padding=(3-1)//2, norm=norm, relu="relu") #논문에 stride = 1/2인데 Transposed이므로 역수로 해주면 된다.
        self.c7s1_3 = ConvBNormRelu2d(nker, out_channels, kernel_size=7, stride=1, padding=(7-1)//2, padding_mode='reflect', norm=None, relu=None) # outchannels = 3
...
class CycleGAN_D(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, norm="inorm"):
        ...
        self.enc5 = ConvBNormRelu2d(in_channels=nker*8, out_channels=out_channels, kernel_size=4, stride=2, padding=1, bias=False, norm=norm, relu="leakyrelu", opts=0.2)
